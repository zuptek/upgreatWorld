document.getElementById('anncX').addEventListener('click',()=>document.getElementById('annc').classList.add('hide'));
  ...
  const fill=(el,list)=>{let s='';for(let k=0;k<2;k++)list.forEach(c=>s+=`<span class="city">${c}</span>`);el.innerHTML=s;};
  ...
  document.querySelector('.burger').addEventListener('click',()=>document.getElementById('services').scrollIntoView({behavior:'smooth'}));
